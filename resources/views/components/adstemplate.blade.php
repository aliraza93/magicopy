<div>
    ...
    <form class="form-inline d-flex mt-3" action="template" method="POST" id="Ad_searchForm" enctype="multipart/form-data">
          @csrf
          <input type="hidden" name="template" value="{{!empty($tempname)?$tempname:'facebook'}}">
    ...
    @if($adscounter >0 && empty($user['subscription']['status']) || !empty($user['subscription']['status']) && $user['subscription']['status']=='active')
        <a href="{{!empty($url)?url($url):url('facebook-ad')}}">
            <div class="sc-iWFSnp cmuvkJ" style="float: left;">
                <div class="sc-dRPiIx esbyIi">
                    <div style="font-size: 45px;">+</div>
                    <div>New {{!empty($tempname)?$tempname:'facebook'}} Ad </div>
                </div>
  ...
  <div id="ADS">
   @foreach($adsinfo as $value)
    <a href="{{!empty($url)?url($url.'/'.$value['company_name'].'/'.encrypt($value['id'])):url('facebook-ad/'.$value['company_name'].'/'.encrypt($value['id']).'')}}" class="ad_class">
...
$( ".remove-ad" ).on( "click", function() {
   ...
   id = $(this).attr('data-id');
     $.ajax({
        ...
        url: "{{url('template')}}",
        type: 'POST',
        data: {template:"{{$tempname=='google'?'google-ads':'facebook-ads'}}",_token:"{{csrf_token()}}"},
        dataType: 'json',
        success: function (data) {
        if(data.response){
             $('.templatediv').html(data.page);
